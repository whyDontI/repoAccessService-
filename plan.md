...
- [ ] `frontend/` — CheckPanel, ExplainView, OrgBrowser
- [x] backend half of this: `backend/Dockerfile` + `postgres`/`backend` services in `docker-compose.yml`, resource-limited (postgres 0.5 vCPU/512MB, backend 1.0 vCPU/1536MB, leaving 0.5 vCPU/2048MB for the frontend once it exists). Fixed the non-idempotent fixture generator (found earlier during oracle harness testing) so the backend container can safely seed on every startup, including against a volume with existing data. Verified with a real clean-slate `docker compose down -v && docker compose up -d --build`: fixture auto-seeds, API responds correctly, restarting the backend container skips re-seeding, full 42-test suite passes against the containerized Postgres. Frontend half (Dockerfile + compose service) waits until the frontend exists.
