...
- [x] `frontend/` — scaffolded with Vite (React + TS, no extra libraries -- matches the original "simple react, no library" call). `CheckPanel.tsx`, `ExplainView.tsx`, `OrgBrowser.tsx`, `TeamTree.tsx`. Added CORS middleware to the backend (`main.py`) since the frontend calls it cross-origin. Known gap: no user search by name -- the API has no endpoint for it, so CheckPanel takes a numeric user id. Could not visually confirm rendering/interactivity -- no browser/screenshot tool in this environment; said so rather than claiming full UI verification.
- [x] Typed contract, upgraded from hand-written to generated: `backend/scripts/export_openapi.py` dumps the OpenAPI schema straight from the Pydantic models (no server needed), `frontend/src/generated-api-types.ts` is generated from that via `openapi-typescript` (`npm run generate-types`), `api.ts` re-exports the generated types instead of duplicating them. Proved the drift-catching actually works by renaming a backend field, regenerating, and watching `tsc -b` fail with an exact file/line -- then reverted the test. Also caught and fixed a real precision gap this surfaced: the old hand-written types claimed `"user" | "team"` literal unions that the backend's plain `str` fields didn't actually guarantee -- fixed at the source with `Literal` types in `models.py`, which now flow through correctly.
...
- [x] `BENCH.md` with real numbers — measured against the actual resource-limited containers, container-to-container (not through the host port, which turned out to add massive non-representative latency on this machine's Docker Desktop networking — a real finding in itself). Found: cold start pays a real one-time cost (16ms on a deep-nesting user's first check, ~0.9ms after), 1000/sec sustains comfortably within budget, but the system doesn't degrade gracefully past ~1500/sec — it collapses (13k+/15k failures at 2000/sec). Neither container ever hit its CPU limit even during collapse, ruling out simple resource exhaustion; named the bottleneck as the single uvicorn worker's connection-handling capacity, tied directly back to the R1/R2 design tradeoff (more workers would break the shared in-memory cache). Also found DB_POOL_MAX_SIZE doesn't behave monotonically across repeated trials (50 was worse than 20, which was better than 10) — kept at 20 with that non-tidy result documented honestly rather than presenting a fake clean optimum.
- [x] finish `README.md`: run instructions, current state, decisions log — done incrementally throughout, plus a final pass adding the collaboration-model section (what was delegated, what was overridden)
- [x] agent transcript exported — raw `.jsonl` pushed to `agent-transcripts/`
