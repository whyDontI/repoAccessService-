...
- [x] `backend/app/checker.py`: `check()` only so far — R1. `explain()` (R3) is next. Added `db.py` (connection pool) and `models.py` (Role enum) to support it. 8 integration tests against a real (separate, disposable) test database, all passing, including the cyclic-team case actually terminating instead of hanging. Spot-checked against the real fixture too: the repo-beats-org case and a cold/warm check on a real depth-8 user, both correct, both well under the 10ms budget (1.7ms cold / 0.7ms warm on a single request — not a real benchmark yet, that's BENCH.md's job).
...
- [ ] `backend/app/oracle.py` — fully separate
- [ ] `backend/scripts/oracle_harness.py` — R4
- [ ] `backend/scripts/load_test.py`
- [ ] `backend/tests/`
- [ ] `frontend/` — CheckPanel, ExplainView, OrgBrowser
- [ ] `docker-compose.yml` + both Dockerfiles, resource-limited
